...
const FEATURED_IDS = [0, 7, 9, 5];
const CATS = ["All", "Content Strategy", "Skincare", "Healthcare", "Health & Nutrition", "Entertainment", "Journalism"];
    ...
    <nav style={{ position:"sticky", top:0, zIndex:100, background:C.white, borderBottom:`1px solid ${C.border}`, padding:"0 48px", height:"60px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
      ...
      <div style={{ display:"flex", gap:"32px", alignItems:"center" }}>
        {[["home","Home"],["portfolio","Portfolio"],["about","About"],["contact","Contact"]].map(([id,label]) => (
          <button key={id} onClick={() => setPage(id)} style={{ background:"none", border:"none", cursor:"pointer", fontFamily:"'Inter',sans-serif", fontWeight:500, fontSize:"13px", color:(page===id || (page==="article" && id==="portfolio")) ? C.dark : C.muted, borderBottom:(page===id || (page==="article" && id==="portfolio")) ? `2px solid ${C.btn}` : "2px solid transparent", paddingBottom:"2px", letterSpacing:"0.01em" }}>
            {label}
          </button>
        ))}
        <button onClick={() => window.open("https://drive.google.com/file/d/1kGXxlNxV6mNG-1YeKSYyZZaG2w3_cT4X/view")} style={{ background:C.btn, color:C.white, border:"none", fontFamily:"'Inter',sans-serif", fontWeight:600, fontSize:"12px", padding:"8px 18px", borderRadius:"6px", cursor:"pointer" }}>
          Resume ↓
        </button>
      </div>
    </nav>
...
function TypeTag({ label }) {
  return (
    <span style={{ fontFamily:"'Inter',sans-serif", fontWeight:500, fontSize:"9px", letterSpacing:"0.08em", textTransform:"uppercase", background:"rgba(30,77,56,0.08)", color:C.mid, padding:"3px 8px", borderRadius:"3px" }}>{label}</span>
  );
}

function SampleCard({ s, setPage }) {
  const handleClick = (e) => {
    if (s.internal) { e.preventDefault(); setPage(s.href); }
  };
  return (
    <div style={{ background:C.light, borderRadius:"10px", padding:"26px 24px", border:`1px solid ${C.border}`, display:"flex", flexDirection:"column", gap:"10px" }}
      ...
      <a href={s.internal ? "#" : s.href} onClick={handleClick} download={s.download} target={(!s.internal && !s.download) ? "_blank" : undefined} rel="noreferrer"
        style={{ fontFamily:"'Inter',sans-serif", fontWeight:600, fontSize:"12px", color:C.mid, textDecoration:"none", marginTop:"4px" }}>
        {s.internal ? "Read Article →" : s.download ? "Download PDF ↓" : "Read Article →"}
      </a>
    </div>
  );
}
...
function Article({ setPage }) {
  return (
    <div style={{ background:C.white, padding:"48px 48px 88px" }}>
      <div style={{ maxWidth:"680px", margin:"0 auto" }}>
        <button onClick={() => setPage("portfolio")} style={{ background:"none", border:"none", cursor:"pointer", fontFamily:"'Inter',sans-serif", fontSize:"13px", color:C.muted, fontWeight:500, marginBottom:"40px", display:"flex", alignItems:"center", gap:"6px", padding:0 }}>
          ← Back to Portfolio
        </button>

        <div style={{ display:"flex", gap:"8px", alignItems:"center", marginBottom:"20px", flexWrap:"wrap" }}>
          <span style={{ fontFamily:"'Inter',sans-serif", fontWeight:700, fontSize:"9px", letterSpacing:"0.1em", textTransform:"uppercase", color:C.mid }}>Content Strategy</span>
          <span style={{ color:C.border }}>·</span>
          <TypeTag label="Thought Leadership" />
        </div>

        <h1 style={{ fontFamily:"'EB Garamond',serif", fontWeight:600, fontSize:"clamp(28px,3.5vw,42px)", color:C.dark, lineHeight:1.15, marginBottom:"20px" }}>
          If Everyone Copies the Same Playbook, How Can We Expect Different Results?
        </h1>

        <div style={{ display:"flex", gap:"16px", alignItems:"center", marginBottom:"48px", paddingBottom:"24px", borderBottom:`1px solid ${C.border}` }}>
          <span style={{ fontFamily:"'Inter',sans-serif", fontSize:"13px", color:C.muted }}>Tanvi Rumale</span>
          <span style={{ color:C.border }}>·</span>
          <span style={{ fontFamily:"'Inter',sans-serif", fontSize:"13px", color:C.muted }}>B2B Content Marketing</span>
          <span style={{ color:C.border }}>·</span>
          <span style={{ fontFamily:"'Inter',sans-serif", fontSize:"13px", color:C.muted }}>2026</span>
        </div>

        {[
          "If you're still writing generic LinkedIn content for your brand in 2026, you're probably not getting anywhere.",
          "Content Marketing 101 says: write about the problems a company targets, pain points, perspectives, and solutions. That advice still holds.",
          "But as more and more companies begin using AI for their content plans and calendars, their periodic LinkedIn posts, and their entire communications, everything blends together, and uniqueness becomes a thing of the past.",
          "We all assumed AI would be an accelerator and, to some extent, a differentiator. But today, the arena has shifted.",
          "LinkedIn has officially been facing an authenticity problem for quite some time now, with over 40% of long-form posts being fully AI-generated. This reminds me of the SEO boom and keyword overstuffing days of the past, when marketers rushed to pull traffic by sacrificing quality for rankings.",
          "All until Google had to step in and make changes. And so it goes, with LinkedIn also stepping in to fight AI slop.",
        ].map((p, i) => (
          <p key={i} style={{ fontFamily:"'Inter',sans-serif", fontSize:"16px", lineHeight:1.8, color:C.text, marginBottom:"22px" }}>{p}</p>
        ))}

        <h2 style={{ fontFamily:"'EB Garamond',serif", fontWeight:600, fontSize:"22px", color:C.dark, margin:"36px 0 16px" }}>So, where do marketers go from here?</h2>

        {[
          "Back to the basics. But this time with one thing in mind: specificity.",
          "HubSpot CMO Kipp Bodnar recommends retracing your steps and really exploring how your brand expresses itself. He suggests: build or rebuild your company's Unique Selling Proposition (USP), so you move with direction and purpose.",
          "Once that's done, companies should redirect all their attention to building personas and profiles to understand their specific pain points, needs, confusions, and preferences, and then designing marketing efforts that directly address those.",
          "Generic content will get you random engagement and will fail to capture your target audience. But speaking directly to your niche group of customers in their own language will get you into their world.",
        ].map((p, i) => (
          <p key={i} style={{ fontFamily:"'Inter',sans-serif", fontSize:"16px", lineHeight:1.8, color:C.text, marginBottom:"22px" }}>{p}</p>
        ))}
        ...
        <blockquote style={{ borderLeft:`4px solid ${C.btn}`, background:C.light, padding:"24px 28px", borderRadius:"0 8px 8px 0", margin:"36px 0" }}>
          <p style={{ fontFamily:"'EB Garamond',serif", fontStyle:"italic", fontSize:"20px", lineHeight:1.6, color:C.dark, marginBottom:"12px" }}>
            "I watched this happen with a luxury chocolate brand I worked on. Their page used to lean on common category adjectives — rich, indulgent, premium — words that every competitor also used."
          </p>
          <p style={{ fontFamily:"'Inter',sans-serif", fontSize:"14px", lineHeight:1.75, color:C.muted }}>
            We flipped this by describing exactly what the target audience — affluent, luxury-experience seeking 35–55-year-olds — would enjoy. "Luxury experiences" and "expertly handcrafted desserts." We started owning a specific idea no one else had claimed.
          </p>
        </blockquote>

        {[
          "We made the generic, specific — and it's the same principle that B2B brands are missing right now. Generic content won't stand out, no matter how good AI gets at generating it.",
          "So, if everyone copies the same playbook, how can we expect different results?",
        ].map((p, i) => (
          <p key={i} style={{ fontFamily:"'Inter',sans-serif", fontSize:"16px", lineHeight:1.8, color:C.text, marginBottom:"22px" }}>{p}</p>
        ))}

        <div style={{ background:C.dark, borderRadius:"10px", padding:"28px 32px", margin:"40px 0" }}>
          <p style={{ fontFamily:"'EB Garamond',serif", fontStyle:"italic", fontSize:"22px", color:C.white, lineHeight:1.5, margin:0 }}>
            "Not by copying the playbook better. By refusing to run it the same way everyone else does."
          </p>
        </div>

        <div style={{ paddingTop:"32px", borderTop:`1px solid ${C.border}`, display:"flex", justifyContent:"space-between", alignItems:"center", flexWrap:"wrap", gap:"12px" }}>
          <span style={{ fontFamily:"'Inter',sans-serif", fontSize:"13px", color:C.muted }}>Written by Tanvi Rumale</span>
          <button onClick={() => setPage("portfolio")} style={{ background:"none", border:`1px solid ${C.border}`, cursor:"pointer", fontFamily:"'Inter',sans-serif", fontSize:"13px", color:C.dark, fontWeight:500, padding:"8px 18px", borderRadius:"6px" }}>
            ← Back to Portfolio
          </button>
        </div>
      </div>
        ...
        <div style={{ maxWidth:"1100px", margin:"0 auto", display:"grid", gridTemplateColumns:"1fr auto", gap:"60px", alignItems:"center" }}>
          <div>
            ...
            <div style={{ display:"flex", gap:"12px", flexWrap:"wrap", marginBottom:"56px" }}>
              <button onClick={() => setPage("portfolio")} style={{ background:C.btn, color:C.white, border:"none", fontFamily:"'Inter',sans-serif", fontWeight:600, fontSize:"14px", padding:"13px 26px", borderRadius:"8px", cursor:"pointer" }}>View Writing Portfolio</button>
              <button onClick={() => setPage("about")} style={{ background:"transparent", color:C.dark, border:`2px solid ${C.dark}`, fontFamily:"'Inter',sans-serif", fontWeight:600, fontSize:"14px", padding:"13px 26px", borderRadius:"8px", cursor:"pointer" }}>About Me</button>
            </div>
      ...
      <section style={{ background:C.offBg, padding:"72px 48px" }}>
        <div style={{ maxWidth:"1100px", margin:"0 auto" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))", gap:"16px", marginBottom:"36px" }}>
            {SAMPLES.filter(s => FEATURED_IDS.includes(s.id)).map(s => <SampleCard key={s.id} s={s} setPage={setPage} />)}
          </div>
    ...
    <div style={{ background:C.white, padding:"64px 48px 88px" }}>
      <div style={{ maxWidth:"1100px", margin:"0 auto" }}>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))", gap:"16px", marginBottom:"64px" }}>
          {filtered.map(s => <SampleCard key={s.id} s={s} setPage={setPage} />)}
        </div>
...
export default function App() {
  ...
  useEffect(() => {
    ...
    style.textContent = `* { margin:0; padding:0; box-sizing:border-box; } html { scroll-behavior:smooth; } body { background:#fff; } input:focus, textarea:focus { border-color:#4D9171 !important; box-shadow:0 0 0 3px rgba(77,145,113,0.1) !important; }`;
  ...
  const pages = {
    ...
    about:     <About />,
    contact:   <Contact />,
    article:   <Article setPage={setPage} />,
  };
